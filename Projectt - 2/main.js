...
// Utility: Convert File Input to Base64 Image string
function readFileAsDataURL(file) {
  return new Promise((resolve) => {
    if (!file) {
      resolve("img/client-1.jpg"); // default avatar
      return;
    }
    const reader = new FileReader();
    reader.onload = (e) => resolve(e.target.result);
    reader.onerror = () => resolve("img/client-1.jpg");
...
// Display Registered User's Name and Profile Avatar
function updateUserNameDisplay() {
  ...
  if (userAvatarElement) {
    userAvatarElement.src = user && user.avatar ? user.avatar : "img/client-1.jpg";
  }
}
...
function addFeedbackToSwiper(name, role, comment, avatar, index) {
  const userAvatar = avatar || "img/client-1.jpg";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (feedbackForm) {
    feedbackForm.addEventListener("submit", (e) => {
      ...
      const userAvatar = loggedUser && loggedUser.avatar ? loggedUser.avatar : "img/client-1.jpg";
